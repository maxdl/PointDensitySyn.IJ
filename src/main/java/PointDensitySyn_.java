/*
    plugin : PointDensitySyn_.java
    author : Max Larsson
    e-mail : max.larsson@liu.se

    This ImageJ plugin is for use in conjunction with PointDensitySyn.py.
...
public class PointDensitySyn_ extends PlugInFrame implements OptionsPDS, 
                                                             ActionListener {
    ...
    Label commentLabel;
    Label scaleLabel;
    ProfileDataPDS profile;
    ImagePlus imp;    

    
    public PointDensitySyn_() {
        super("PointDensitySyn");
        if (instance != null) {
            instance.toFront();
            return;
        }
        instance = this;
        profile = new ProfileDataPDS();
        IJ.register(PointDensitySyn_.class);
    ...
    PolygonRoi getPolygonRoi(ImagePlus imp) {
        ...
        if (roi == null || roi.getType() != Roi.POLYGON) {
            IJ.error("PointDensitySyn", "Polygon selection required.");
            return null;
        } else {
	...
	PolygonRoi getPolylineRoi(ImagePlus imp) {
		...
		if (roi == null || roi.getType() != Roi.POLYLINE) {
			IJ.error("PointDensitySyn", "Segmented line selection required.");
			return null;
		} else {
			return (PolygonRoi) roi; 
		}
    ...
    PolygonRoi getPointRoi(ImagePlus imp) {
        ...
        if (roi == null || roi.getType() != Roi.POINT) {
            IJ.error("PointDensitySyn", "Point selection required.");
            return null;
        } else {
            return (PolygonRoi) roi;
        }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (command.equals("Plasma membrane")) {
            if (!isImage(imp) || !profile.isSameImage(imp) ||
                    profile.isDefined("plasma membrane", "Plasma membrane")) {
                return;
            }
        ...
        if (command.equals("Delete postsynaptic density")) {
            ...
            profile.deleteNamedComponent(imp, "psd");
        }
...
} // end of PointDensitySyn_


class ProfileDataPDS implements OptionsPDS {
    boolean dirty;
    Overlay overlay;
    ...
    public void deleteSelectedComponent(ImagePlus imp) {
        if (!this.overlay.contains(imp.getRoi())) {
            ...
            YesNoCancelDialog d = new YesNoCancelDialog(imp.getWindow(),
                    "PointDensitySyn", "Delete " + imp.getRoi().getName() + "?");
            if (d.yesPressed()) {
    ...
    public boolean isSameImage(ImagePlus imp) {
        if (!this.dirty || this.imgID == 0) {
            ...
            IJ.error("PointDensitySyn", "All measurements must be performed on the same image.");
            return false;
        }
    }
    ...
    private boolean CheckProfileData(ImagePlus imp) {
        ...
        if (this.getNumPoints("plasma membrane") == 0) {
            errstr[nerr++] = "Plasma membrane not defined.";
        }
        if (this.getNum("psd") == 0) {
            errstr[nerr++] = "PSD/active zone not defined.";
        }
        if (this.getNumPoints("points") == 0) {
            warnstr[nwarn++] = "No point coordinates defined.";
        }
        if (nerr > 0) {
            IJ.error("PointDensitySyn", "Error:\n" + errstr[0]);
            return false;
        }
        if (nwarn > 0) {
            for (i = 0; i < nwarn; i++) {
                YesNoCancelDialog d = new YesNoCancelDialog(imp.getWindow(),
                        "PointDensitySyn", "Warning:\n" + warnstr[i] + "\nContinue anyway?");
                if (!d.yesPressed()) {
                    return false;
                }
            }
    ...
    public boolean save(ImagePlus imp) {
        ...
        try {
            ...
            outf.println("PLASMA_MEMBRANE");
            pol = this.overlay.get(this.overlay.getIndex("plasma membrane")).getPolygon();
    ...
    public static void main(String[] args) {
        ...
        Class<?> cl = PointDensitySyn_.class;
